...
public class Graph {
    ...
    private final int FINAL_VERTEX = 6;
    ...
    private void displayPaths() {
        ...
        for (int i = 0; i < countOfVertices; i++){
            if (i == (FINAL_VERTEX - 1)) {
                System.out.println(vertexList[i].getLabel() + " составляет: ");
                StringBuilder result = new StringBuilder(shortestPaths.get(i).getDistance() + " шага: ");
                List<Integer> parents = shortestPaths.get(i).getParentVertices();
                for (int j = 0; j < parents.size(); j++) {
                    result.append(vertexList[parents.get(j)].getLabel()).append(" -> ");
                }
                System.out.println(result.append(vertexList[i].getLabel()));
            }
        }
    }
}
